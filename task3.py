...
def main():
    ...
    # Optional: Add a final segment to return to the home position
    return_to_home = rospy.get_param('~return_to_home', False)
    ...
    out_file = rospy.get_param('~output_file', 'traj_all_joints.png')
    ...
    if return_to_home:
        # Need to unwrap the home position relative to the last target
        # Calculate both solutions for home and choose closest to the LAST via point
        x_home_cart = L1 + L2 + L3
        y_home_cart = 0.0
        phi_home_cart = 0.0
        ok, q_down_raw, q_up_raw = inverse_kinematics_with_orientation(x_home_cart, y_home_cart, phi_home_cart)

        if not ok:
            rospy.logfatal('IK failed for return to home position. Aborting.')
            return

        q_down_unwrapped = unwrap_solution(q_down_raw, joint_targets[-1])
        q_up_unwrapped   = unwrap_solution(q_up_raw, joint_targets[-1])

        dist_down = np.sum((q_down_unwrapped - joint_targets[-1])**2)
        dist_up   = np.sum((q_up_unwrapped - joint_targets[-1])**2)

        if dist_down < dist_up:
            q_chosen_home = q_down_unwrapped
            rospy.loginfo(f"Return Home: Chose Elbow-Down solution. Distance: {dist_down:.4f}")
        else:
            q_chosen_home = q_up_unwrapped
            rospy.loginfo(f"Return Home: Chose Elbow-Up solution. Distance: {dist_up:.4f}")

        joint_targets.append(q_chosen_home)
        rospy.loginfo(f"Added 'return to home' pose (final): ({q_chosen_home[0]:.3f}, {q_chosen_home[1]:.3f}, {q_chosen_home[2]:.3f})")
    ...
    # --- All segments done ---
    # Create time points for via points plot
    # These are the actual times at which the robot *reaches* the via points
